...
The policies differ in how they use `--outer-loops`. With `cyclic`, one outer
loop scans all coordinate blocks generated by `--block-size` and
`--block-stride`. With `gradient` and `hessian`, each outer loop selects only
one active block after re-linearizing around the current window state. Use more
outer loops, for example `--outer-loops 10` to `20`, when running adaptive block
selection.
...
- `--outer-loops`: repeated block passes per window. For `cyclic`, each loop
  scans all cyclic blocks; for `gradient` and `hessian`, each loop selects and
  optimizes one active block.
- `--bits-per-dim`: binary resolution per state dimension.
- `--radius`: maximum local increment magnitude encoded by the QUBO grid.
- `--time-sweeps`: number of full passes over all sliding windows. Later sweeps
  default to continuing from each window's previous optimized initial state.
- `--time-sweep-mode`: `carry` keeps optimizing each window's carried `x0`
  across time sweeps; `background` rebuilds each window background from the
