...
for outer_loop in 1..N:
    select blocks for this loop
    # cyclic: all coordinate blocks
    # gradient/hessian: one active block
...
当前实现还支持两类 sweep。第一类是 \texttt{outer\_loops}，即在同一个时间窗口内反复执行 block 更新。对 \texttt{cyclic} 策略而言，一个 outer loop 会按坐标顺序扫描所有 block；对 \texttt{gradient} 和 \texttt{hessian} 策略而言，一个 outer loop 只会在当前线性化模型下选择并优化一个最活跃的 block，然后下一轮 outer loop 在更新后的状态附近重新线性化并重新选块。因此，自适应 block 选择通常应配合更多的 \texttt{outer\_loops}，例如 10 到 20 轮，而不是在一轮中一次性取很多自适应 block。第二类是 \texttt{time\_sweeps}，即在完成一次从前到后的所有时间窗口同化后，从第一个窗口重新开始新一轮时间 sweep。当前默认采用 \texttt{time\_sweep\_mode=carry}：每个窗口保存上一轮优化得到的窗口初值 $x_0$，下一轮回到同一个窗口时直接在该 $x_0$ 附近继续线性化和更新；也可以切换到 \texttt{background} 模式，即从当前分析轨迹中重新构造窗口背景。前者更贴近“继续优化窗口初值”的增量 4D-Var 解释，后者则更像重跑一次滑动窗口背景传播。继续增大 \texttt{outer\_loops} 或 \texttt{time\_sweeps} 会线性增加 QUBO 调用次数。
...
这两种自适应策略对应了 active coordinate selection 和 Hessian-induced coupling graph 的思想。前者把量子调用集中在当前最可能降低代价函数的坐标上；后者进一步考虑 4D-Var Hessian 中的变量相互作用，尽量把强耦合变量放入同一个 QUBO 子问题。在固定量子比特预算下，这相当于让每次 QUBO 调用优先覆盖“误差大且相互影响强”的局部子空间，而不是机械地扫描坐标顺序。由于每次自适应选择都依赖当前状态，本文实现中 \texttt{gradient} 和 \texttt{hessian} 每个 outer loop 只产生一个 block。
