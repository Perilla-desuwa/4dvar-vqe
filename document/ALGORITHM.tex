...
\title{Lorenz96 的量子辅助增量 4D-Var 方法说明}
\author{}
\date{\today}
...
其中 \texttt{block\_stride} 可以小于 \texttt{block\_size}，从而产生重叠 block。例如：
\begin{equation}
  \mathrm{block\_size}=10,
  \qquad
  \mathrm{block\_stride}=5,
\end{equation}
对应的 block 为
\begin{equation}
  0..9,\quad 5..14,\quad 10..19,\quad \ldots
\end{equation}

重叠 block 可以减轻硬切分状态维度带来的边界效应，也更适合 Lorenz96 这类局部耦合系统。
...
当前实现还支持 \texttt{outer\_loops}，即在同一个时间窗口内反复 sweep 所有 block。这样做相当于 block-coordinate descent 的多轮迭代：第一轮得到粗略修正，后续轮次在更新后的状态附近继续修正。一般来说，\texttt{outer\_loops=2} 或 3 可能带来收益，但计算量也会相应增加。

综合来看，优化维度选择需要在以下三者之间平衡：
\begin{itemize}
  \item 单次 QUBO 变量数：由 \texttt{block\_size} 和 \texttt{bits\_per\_dim} 决定；
  \item 状态耦合覆盖：由 \texttt{block\_stride} 决定；
  \item 计算时间：由 block 数量、窗口数和 \texttt{outer\_loops} 共同决定。
\end{itemize}

\subsection{QAOA 后端}

QAOA 后端会将 QUBO 转换为 Ising 哈密顿量。对二进制变量 $q_i$，使用映射
\begin{equation}
  q_i = \frac{1 - z_i}{2}.
\end{equation}
...
实现中使用 Qiskit Aer 对线路采样，并从采样得到的 bitstring 中选择 QUBO 能量最低的解。代码也支持用 COBYLA 调整 QAOA 角度；不过在端到端大规模实验中，固定浅层角度通常更快。
